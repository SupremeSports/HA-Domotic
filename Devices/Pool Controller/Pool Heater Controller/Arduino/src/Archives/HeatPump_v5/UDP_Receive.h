...
void callSubfunction()
{
  ...
  else if(command == "heatset")         //Receive temperature command from server
  {
    ...
    temp_req = constrain(temp_req, 69+1, TEMP_MAX);
  ...
  else if(command == "heatmode")        //Switch SPA/POOL modes
  {
  ...
  else if(command == "status")          //Send heat pump system status to server
  {
    sprintf(str, "%s;%s;%s;%s;%d", 
      (pump_out ? "RUN" : "STOP"),String(temp_req,1).c_str(),String(temp_in_F,1).c_str(),String(temp_out_F,1).c_str(), (alarm_on ? "ALARM" : "SYS OK"), alarm_bits);
  }
  else
  {
    sprintf(str, "%s", "ERROR");
