...
void callSubfunction()
{
  ...
  {
    int device = commands[1].toInt();
    bool enet_switch = commands[2] == "0" ? false : true;
    int enet_value = enet_switch ? commands[2].toInt() : 0;
    if(device <= 2)
    {
      Enet_Pot_Mtr[device] = enet_value;
      Enet_Sw_Mtr[device] = enet_switch;
    }
    else if(device == 3)
    {
      heat_enable = (commands[2] == "1") ? true : false;
      temp_req = heat_enable && (temp_req < TEMP_MIN+1) ? TEMP_DEF : temp_req;
      temp_req = !heat_enable ? TEMP_MIN : temp_req;
      show.temp_chg_base();
      show.temp_chg();
    }
    else if(device == 4)
      heatpump_enable = enet_switch;
    else if(device == 5)
      solar_enable = enet_switch;
