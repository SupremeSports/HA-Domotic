...
//*  nixie@protonmail.ch                                                           *
//*                                                                                *
//* Modified by SuprmeeSports - Jean Gauthier P.Eng                                *
//*   - Modified the pinout for display tubes upside down                          *
//*                                                                                *
//**********************************************************************************
//**********************************************************************************
// Standard Libraries
...
#define EE_BACKLIGHT_MODE               15     // The back light mode
...
#define COLOUR_GRN_CNL_DEFAULT          15
...
#define MODE_DIGIT_BURN                 99 // Digit burn mode - accessible by super long press

// Temporary display modes - accessed by a short press ( < 1s ) on the button when in MODE_TIME
#define TEMP_MODE_MIN                   0
#define TEMP_MODE_DATE                  0 // Display the date for 5s
#define TEMP_MODE_TEMP                  1 // Display the temperature for 5s
...
void setup()
{
  ...
  pinMode(inputPin1, INPUT_PULLUP); // Set the input pin 1
  ...
  if (button1.isButtonPressedNow())
  {
    ...
    // Flash some ramdom colours to signal that we have accepted the factory reset
    for (int i = 0; i < 60; i++)
      randomRGBFlash(20);
  ...
  // Mark that we have done the EEPROM setup
  EEPROM.update(EE_NEED_SETUP, false);
}
...
void loop()
{
  ...
  if (button1.isButtonPressed2S())
    nextMode = MODE_MIN;  // Just jump back to the start
  ...
  if ((currentMode != MODE_DIGIT_BURN) && (nextMode != MODE_DIGIT_BURN))
  {
    ...
    digitOn(digitBurnDigit, digitBurnValue);  // Digit burn mode
...
void performOncePerMinuteProcessing()
{
  if (useWiFi > 0)
  {
    if (useWiFi == MAX_WIFI_TIME)
      setRTC();   // We recently got an update, send to the RTC (if installed)
      
    useWiFi--;
  } 
  else
    getRTCTime(); // Get the time from the external RTC provider - (if installed)
}
  ...
  switch (currentMode)
  {
    case MODE_TIME:
    {
      if (button1.isButtonPressedAndReleased())
      {
        ...
        {
          // Apply 5 sec timeout for setting the suppression time
      ...
      if (tempDisplayModeDuration > 0)
      {
        ...
        if (tempDisplayMode == TEMP_IP_ADDR12)
        {
          if (useWiFi > 0)
            ...
            tempDisplayMode++;  // We can't show the IP address if we have the RTC, just skip
        }
...
{
  uint16_t = yearAdjusted = year() % 100;
  switch (dateFormat)
  {
    case DATE_FORMAT_YYMMDD:
      ...
      NumberArray[1] = yearAdjusted % 10;
      NumberArray[0] = yearAdjusted / 10;
      break;
    case DATE_FORMAT_MMDDYY:
      NumberArray[5] = yearAdjusted % 10;
      NumberArray[4] = yearAdjusted / 10;
...
void outputDisplay()
{
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    if ((NumberArray[i] != currNumberArray[i]) && (NumberArray[i] == 0) && scrollback)
      tmpDispType = SCROLL;
    ...
    if (tmpDispType == SCROLL)
    {
      ...
      {
        if (fadeState[i] == 0)
          fadeState[i] = scrollSteps; // Start the fade
        ...
        if (fadeState[i] == 1)
        {
          ...
          fadeState[i] = fadeState[i] - 1;  // Continue the scroll countdown
      }
    }
...
{
  int decodedPosition = decodePosition[digit];
  switch (decodedPosition)
  {
    case 0: PORTD = PORTD | B00000001; break; // PD0 - equivalent to digitalWrite(ledPin_a_6,HIGH);
    case 1: PORTD = PORTD | B00000010; break; // PD1 - equivalent to digitalWrite(ledPin_a_5,HIGH);
    case 2: PORTD = PORTD | B00000100; break; // PD2 - equivalent to digitalWrite(ledPin_a_4,HIGH);
    case 3: PORTD = PORTD | B00010000; break; // PD4 - equivalent to digitalWrite(ledPin_a_3,HIGH);
    case 4: PORTC = PORTC | B00000100; break; // PC2 - equivalent to digitalWrite(ledPin_a_2,HIGH);
    case 5: PORTC = PORTC | B00001000; break; // PC3 - equivalent to digitalWrite(ledPin_a_1,HIGH);
  }
...
{
  // Check day blanking, but only when we are in normal time mode
  if (currentMode == MODE_TIME)
  {
    switch (dayBlanking)
    {
...
boolean getHoursBlanked()
{
  if (blankHourStart > blankHourEnd)
    return ((hour() >= blankHourStart) || (hour() < blankHourEnd)); // Blanking before midnight
  else if (blankHourStart < blankHourEnd)
    return ((hour() >= blankHourStart) && (hour() < blankHourEnd)); // Dim at or after midnight
  else
    return false; // No dimming if Start = End
}
...
{
  EEPROM.update(EE_12_24, hourMode);
  EEPROM.update(EE_FADE_STEPS, fadeSteps);
  EEPROM.update(EE_DATE_FORMAT, dateFormat);
  EEPROM.update(EE_DAY_BLANKING, dayBlanking);
  EEPROM.update(EE_DIM_DARK_LO, dimDark % 256);
  EEPROM.update(EE_DIM_DARK_HI, dimDark / 256);
  EEPROM.update(EE_BLANK_LEAD_ZERO, blankLeading);
  EEPROM.update(EE_SCROLLBACK, scrollback);
  EEPROM.update(EE_FADE, fade);
  EEPROM.update(EE_SCROLL_STEPS, scrollSteps);
  EEPROM.update(EE_DIM_BRIGHT_LO, dimBright % 256);
  EEPROM.update(EE_DIM_BRIGHT_HI, dimBright / 256);
  EEPROM.update(EE_DIM_SMOOTH_SPEED, sensorSmoothCountLDR);
  EEPROM.update(EE_RED_INTENSITY, redCnl);
  EEPROM.update(EE_GRN_INTENSITY, grnCnl);
  EEPROM.update(EE_BLU_INTENSITY, bluCnl);
  EEPROM.update(EE_BACKLIGHT_MODE, backlightMode);
  EEPROM.update(EE_HV_VOLTAGE, hvTargetVoltage);
  EEPROM.update(EE_SUPPRESS_ACP, suppressACP);
  EEPROM.update(EE_HOUR_BLANK_START, blankHourStart);
  EEPROM.update(EE_HOUR_BLANK_END, blankHourEnd);
  EEPROM.update(EE_CYCLE_SPEED, cycleSpeed);
  EEPROM.update(EE_PULSE_LO, pwmOn % 256);
  EEPROM.update(EE_PULSE_HI, pwmOn / 256);
  EEPROM.update(EE_PWM_TOP_LO, pwmTop % 256);
  EEPROM.update(EE_PWM_TOP_HI, pwmTop / 256);
  EEPROM.update(EE_MIN_DIM_LO, minDim % 256);
  EEPROM.update(EE_MIN_DIM_HI, minDim / 256);
  EEPROM.update(EE_ANTI_GHOST, antiGhost);
  EEPROM.update(EE_USE_LDR, useLDR);
  EEPROM.update(EE_BLANK_MODE, blankMode);
  EEPROM.update(EE_SLOTS_MODE, slotsMode);
}
...
int getDimmingFromLDR()
{
  if (useLDR)
  {
    ...
    if (returnValue < minDim)
      returnValue = minDim;
    if (returnValue > DIGIT_DISPLAY_OFF)
      returnValue = DIGIT_DISPLAY_OFF;
    return returnValue;
  }
  else
...
void calibrateHVG()
{
  ...
  for (int i = 0; i < 768; i++)
  {
    ...
    if ((getSmoothedHVSensorReading() < rawHVADCThreshold) && (i % 8) == 0)
      incPWMOnTime();
...
void receiveEvent(int bytes)
{
  ...
  else if (operation == I2C_SET_OPTION_MIN_DIM)
  {
    ...
    EEPROM.update(EE_MIN_DIM_HI, dimHI);
    EEPROM.update(EE_MIN_DIM_LO, dimLO);
  }
}
...
{
  byte byteToSend = valueToProcess ? 1 : 0;
  return byteToSend;
}
